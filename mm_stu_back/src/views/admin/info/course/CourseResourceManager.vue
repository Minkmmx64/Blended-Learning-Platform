<template>
  <div class="w-full h-full scroll">
    ...
    >
      <el-row class="cell mb-5 text-center flex-row flex-center">
        <el-col :span="6">
      ...
      <el-row class="cell mb-5 text-center flex-row flex-center">
        <el-col :span="6">
          <div class="h-full flex-row flex-center">
            <span>{{ TableProps.apiname }}所属课程:</span>
          </div>
        </el-col>
        <el-col :span="18">
          <el-select
              v-model="allDatas.courseId"
              class="m-2"
              placeholder="select course"
          >
            <el-option
                v-for="item in allDatas.courses"
                :key="item.id"
                :label="item.name"
                :value="item.id"
            />
      ...
      <el-row class="cell mb-5 text-center flex-row flex-center" v-if="allDatas.courseId">
        <el-col :span="6">
          <div class="h-full flex-row flex-center">
            <span>{{ TableProps.apiname }}所属章节:</span>
          </div>
        </el-col>
        <el-col :span="18">
          <el-tree-select
              v-model="EditParams.chapter_id"
              :data="allDatas.chapters"
      ...
      <el-row class="cell mb-5 text-center flex-row flex-center">
        <el-col :span="6">
          <div class="h-full flex-row flex-center">
            <span>{{ TableProps.apiname }}类型:</span>
          </div>
        </el-col>
        <el-col :span="18">
          <el-input
      ...
      <el-row class="cell mb-5 text-center flex-row flex-center">
        <el-col :span="6">
          <div class="h-full flex-row flex-center">
            <span>{{ TableProps.apiname }}路径:</span>
          </div>
        </el-col>
        <el-col :span="18">
          <el-input
              style="width: 200px"
              v-model="EditParams.src"
              placeholder="resource src"
          />
        </el-col>
      </el-row>
      <template #footer>
...
import { resource, resourcedata } from "@/Request/ApiModules/resource";
import course from "@/Request/ApiModules/course";
import chapter from "@/Request/ApiModules/chapter";
...
import UseElUploadFiles from "@/components/display/ElUploadFiles.vue";
...
const TableProps = useTableFunction(
    ...
    undefined,
    {
      beforehandleEditOpen(row) {
        if(row) {
          allDatas.courseId = row.chapter.course.id
        }
      }
    },
    resourcedata
...
const loadCourses = () => {
  course.all().then( res => {
    allDatas.courses = res.data.data;
  })
}

watchEffect(() => {
  if(allDatas.courseId !== undefined) {
    loadChapters();
  }
})

const loadChapters = () => {
  chapter.getChapterByCourseId(allDatas.courseId)
         .then(res => allDatas.chapters = res.data.data)
